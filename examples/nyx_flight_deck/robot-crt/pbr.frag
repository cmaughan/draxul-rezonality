...
{
    // Use the exact HDR radiance function displayed by the sky pass so the
    // robot reflects and inherits the surrounding neon colors.
    return neonEnvironment(dir, ubo.iTime * 2.0) * 2.4;
}
...
void main()
{
    ...
    vec3 direct = (kd * albedo / PI + specular) * vec3(1.7, 1.2, 2.1) * ndotl;

    vec3 reflected = reflect(-v, n);
    vec3 envDiffuse = sampleEnvironment(n) * albedo * occlusion * 0.28;
    vec3 envSpecular = sampleEnvironment(reflected) * fresnelSchlick(ndotv, f0) * (1.0 - roughness * 0.78) * 1.35;
