...
struct NativeBackend::Impl
{
    ...
    std::optional<VulkanGeneration> prepared;
...
void NativeBackend::bind_frame(const DraxulPluginVulkanFrameV2& frame,
...
void NativeBackend::retire_completed_slot(uint32_t frame_index)
{
    ...
    for (auto iterator = impl_->backend.retired.begin();
         iterator != impl_->backend.retired.end();)
    {
        if (iterator->pending_slots == 0)
        {
            destroy_generation(iterator->generation);
            iterator = impl_->backend.retired.erase(iterator);
        }
        else
            ++iterator;
    }
...
DraxulPluginRenderResultV2 NativeBackend::record(
    const DraxulPluginVulkanFrameV2& bound_frame,
    const AudioTextureFrame* audio, bool paused)
{
    ...
    const VkRenderPass render_pass = reinterpret_cast<VkRenderPass>(
        static_cast<uintptr_t>(frame->continuation_render_pass));
    if (!impl_->backend.active
        || impl_->backend.active->target_generation
    ...
    std::memcpy(static_cast<uint8_t*>(
                    impl_->backend.active->uniform_buffer.mapped)
            + uniform_offset,
        uniform.data(), sizeof(uniform));
    vmaFlushAllocation(impl_->backend.active->allocator,
        impl_->backend.active->uniform_buffer.allocation,
        uniform_offset, sizeof(uniform));
    const VkDeviceSize offset = 0;
    for (auto& pass : impl_->backend.active->passes)
    {
        if (pass.ray_trace)
        {
        ...
        uint32_t target_width = impl_->backend.active->width;
        uint32_t target_height = impl_->backend.active->height;
        int32_t target_x = 0;
        int32_t target_y = 0;
        ...
        {
            vkCmdBindVertexBuffers(command, 0, 1,
                &impl_->backend.vertex_buffer, &offset);
